...
void HalfEdgeMesh::CopyHalfEdgeMesh(HalfEdgeMesh* hem){
	...
	//std::vector<HalfEdgeMesh*> hem_he;

	for(int i = 0; i < n_v; i++){
		hem->p_v[i] = new Vertex(p_v[i]->Pos);
		//Vertex* tmp_v = new Vertex(p_v[i]->Pos);
		//hem.p_v.push_back(tmp_v);
		hem->p_v[i]->Id = i;
	...
	for(int i = 0; i < n_f; i++){
		...
		do{
			...
			int v_id = he->p_v->Id;
			int e_id = he->p_e->Id;
			//HalfEdge* tmp_he = new HalfEdge(hem.p_v[v_id]);
			//tmp_he->p_f = hem.p_f[f_id];
			//tmp_he->p_e = hem.p_e[e_id];
			//tmp_he->IAng = he->IAng;
			//tmp_he->Cotan = he->Cotan;
			//hem_he.push_back(tmp_he);
			//hem.p_he.push_back(tmp_he);
			hem->p_he[he_id] = new HalfEdge(hem->p_v[v_id]);
			hem->p_he[he_id]->Id = he_id;
			//hem.p_he[he_id]->p_f = hem.p_f[i];
			//hem.p_he[he_id]->p_e = hem.p_e[e_id];
			hem->p_he[he_id]->IAng = p_he[he_id]->IAng;
			hem->p_he[he_id]->Cotan = p_he[he_id]->Cotan;
			if(j == 0){
				hem->p_f[i] = new Face(hem->p_he[he_id]);
				//Face* tmp_f = new Face(hem.p_he[he_id]);
				//hem.p_f.push_back(tmp_f);
	...
	for(int i = 0; i < n_he; i++){
		//std::cout << i << std::endl;
		//std::cout << "p_he->Id: " << p_he[i]->Id << std::endl;
		//std::cout << "pair: " << p_he[i]->pair->Id << std::endl;
		//std::cout << "next: " << p_he[i]->next->Id << std::endl;
		//std::cout << "prev: " << p_he[i]->prev->Id << std::endl;
	...
	for(int i = 0; i < n_e; i++){
		...
		//Edge* tmp_e = new Edge(hem.p_he[p_e[i]->p_he_left->Id]);
		//hem.p_e.push_back(tmp_e);
	}
	...
	//std::cout << "copy_boundary:\n";
	for(int i = 0; i < n_bnd; i++){
		hem->p_bv[i] = hem->p_v[p_bv[i]->Id];
		/*
		std::cout << hem->p_bv[i]->Id 
			<< " (" 
			//<< hem->p_bv[i]->p_he->prev->p_v->Id 
			<< "*"
			<< " -> " 
			<< hem->p_bv[i]->p_he->p_v->Id 
			<< " -> " 
			<< hem->p_bv[i]->p_he->next->p_v->Id 
			<< ")" << std::endl;
		*/
	}
}
void HalfEdgeMesh::ReadTriangleMesh(std::string fname){
	Eigen::MatrixXd V;
	Eigen::MatrixXi F;
	...
	for(int i = 0 ; i < n_v; i++){
		Vertex* tmp_v = new Vertex(V(i, 0), V(i, 1), V(i, 2));
		p_v.push_back(tmp_v);
	}

	for(int i = 0; i < n_f; i++){
		AddFace(p_v[F(i, 0)], p_v[F(i, 1)], p_v[F(i, 2)]);
	}
	...
	for(int i = 0; i < n_bnd; i++){
		...
		std::cout << p_bv[i]->Id 
			...
			<< p_bv[i]->p_he->p_v->Id 
			<< " -> " 
			<< p_bv[i]->p_he->next->p_v->Id 
			<< ")" << std::endl;
	}
	*/
}

void HalfEdgeMesh::WriteTriangleMesh(std::string fname){
	Eigen::MatrixXd V;
	Eigen::MatrixXi F;
	...
	SetVertexIds();
	for(int i = 0; i < n_f; i++){
		F(i, 0) = p_f[i]->p_he->p_v->Id;
		F(i, 1) = p_f[i]->p_he->next->p_v->Id;
		F(i, 2) = p_f[i]->p_he->prev->p_v->Id;
	}
...
void HalfEdgeMesh::CalcCentroid(Face* f){
	Eigen::Vector3d v0 = f->p_he->p_v->Pos;
	Eigen::Vector3d v1 = f->p_he->next->p_v->Pos;
	Eigen::Vector3d v2 = f->p_he->prev->p_v->Pos;
	f->Centroid = (v0 + v1 + v2) / 3.0;
}
...
void HalfEdgeMesh::CalcCentroids(){
	int n_f = (int)p_f.size();
//#ifdef _OPENMP
//#pragma omp parallel for num_threads(NumThread)
//#endif
...
void HalfEdgeMesh::MapBoundaryToCircle(HalfEdgeMesh* hem){
	...
	//std::cout << "p_bv_loop: " << p_bv_loop[0]->Id << " " << std::flush;
	for(int i = 0; i < n_bnd - 1; i++){
		Vertex* tmp = p_bv_loop[i]->p_he->next->p_v; // 境界辺のp_heは境界辺に沿ったハーフエッジにしている
		p_bv_loop.push_back(tmp);
		//std::cout << p_bv_loop[i+1]->Id << " ";
	}
	//std::cout << std::endl;

	/*
	for(int i = 0; i < n_bnd; i++){
		Eigen::Vector3d e = p_bv_loop[(i+1)%n_bnd]->Pos - p_bv_loop[i]->Pos;
		len[i] = e.norm();
		sum_len += len[i];
	}

	hem->p_bv[0]->Pos(2) = 0.0;
	double tmp_radius = hem->p_bv[0]->Pos.norm();
	hem->p_bv[0]->Pos = hem->p_bv[0]->Pos / tmp_radius;
	double theta0 = atan2(hem->p_bv[0]->Pos(1), hem->p_bv[0]->Pos(0));

	double thetai = theta0;
	double sum_delta = 0.0;
	for(int i = 1; i < n_bnd; i++){
		double delta = 2.0 * M_PI * len[i] / sum_len;
		sum_delta += delta;
		thetai += delta;
		std::cout << hem->p_v[p_bv_loop[i]->Id]->Id << ": " << thetai * 360.0 / (2.0 * M_PI) << "  delta: " << delta * 180.0 / M_PI << std::endl;
		hem->p_v[p_bv_loop[i]->Id]->Pos(0) = cos(thetai);
		hem->p_v[p_bv_loop[i]->Id]->Pos(1) = sin(thetai);
	}
	std::cout << "sum_delta: " << sum_delta * 180.0 / M_PI << std::endl;
	*/
...
void HalfEdgeMesh::HarmonicParam(HalfEdgeMesh* hem){
	...
	// check ---------------
	//hem->WriteTriangleMesh("test_copy.off");
	// check ---------------
	...
	for(int i = 0; i < n_bnd; i++){
		bv.push_back(p_bv[i]->Id);
		//std::cout << "boundary: " << p_bv[i]->Id << " " << p_bv[i]->Pos(0) << " " << p_bv[i]->Pos(1) << std::endl;
	}
...
#pragma omp section
{
	...
	// check ---------------
	//hem->WriteTriangleMesh("test_param.off");
	// check ---------------
}

void HalfEdgeMesh::SeparateByTriangle(HalfEdgeMesh* hem){
	//int n_v_ref = (int)p_v.size();
	int n_f_ref = (int)p_f.size();
	//int n_e_ref = (int)p_e.size();
	//int n_he_ref = (int)p_he.size();
	//int n_bnd_ref = (int)p_bv.size();

	for(int i = 0; i < n_f_ref; i++){
		HalfEdge* he = p_f[i]->p_he;
		do{
			Vertex* tmp_v = new Vertex(he->p_v->Pos);
			hem->p_v.push_back(tmp_v);

			he = he->next;
		}while(he != p_f[i]->p_he);
		//std::cout << i << std::endl;
		hem->AddFace(hem->p_v[3*i], hem->p_v[3*i+1], hem->p_v[3*i+2]);
		//std::cout << i << std::endl;
	}
